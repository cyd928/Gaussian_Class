...
class Gaussian():
    ...
    def calculate_stdev(self, sample=True):
        ...
        """
        if sample:
            squared_sum = sum([(x - self.calculate_mean()) ** 2 for x in self.data])
            self.stdev = math.sqrt(squared_sum / (len(self.data) - 1))

        else:
            squared_sum = sum([(x - self.calculate_mean()) ** 2 for x in self.data])
            self.stdev = math.sqrt(squared_sum / len(self.data))
    ...
    def plot_histogram(self):
        ...
        plt.title('Histogram of Data List')
        plt.show()
